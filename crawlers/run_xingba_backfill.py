...
from spiders.xingba_forums import XINGBA_FORUM_BY_SLUG, XINGBA_FORUMS
...
def main():
    ...
    if not slug:
        raise SystemExit(f"--slug required. Choices: {', '.join(slugs)}")
    ...
    forum = XINGBA_FORUM_BY_SLUG.get(slug)
    if not forum:
